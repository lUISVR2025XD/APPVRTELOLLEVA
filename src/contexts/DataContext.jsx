  ...
  useEffect(() => {
    const handleInserts = (payload) => {
      ...
      switch (table) {
        ...
        case 'businesses': setBusinesses(updater); break;
        case 'delivery_persons': setDeliveryPersons(updater); break;
        case 'products': setProducts(updater); break;
        case 'orders': setOrders(updater); break;
        case 'order_history': setOrderHistory(updater); break;
        case 'quick_messages': setQuickMessages(updater); break;
        default: break;
      }
    };

    const handleUpdates = (payload) => {
      const { table, new: updatedRecord } = payload;
      const updater = (prev) => prev.map(item => item.id === updatedRecord.id ? updatedRecord : item);
      switch (table) {
        case 'clients': setClients(updater); break;
        case 'businesses': setBusinesses(updater); break;
        case 'delivery_persons': setDeliveryPersons(updater); break;
    ...
    const handleDeletes = (payload) => {
      ...
      switch (table) {
        case 'clients': setClients(remover); break;
        case 'businesses': setBusinesses(remover); break;
        case 'delivery_persons': setDeliveryPersons(remover); break;
        case 'products': setProducts(remover); break;
        case 'orders': setOrders(remover); break;
        case 'quick_messages': setQuickMessages(remover); break;
        default: break;
      }
    };
  ...
  const addRating = async (order, ratings) => {
    ...
    if (ratingInsertError) {
      ...
      if (business_rating > 0) {
        const { data: businessRatings, error: brError } = await supabase.from('ratings').select('business_rating', { count: 'exact' }).eq('business_id', order.business_id).not('business_rating', 'is', null);
      ...
      if (delivery_rating > 0 && order.delivery_person_id) {
        const { data: deliveryRatings, error: drError } = await supabase.from('ratings').select('delivery_rating', { count: 'exact' }).eq('delivery_person_id', order.delivery_person_id).not('delivery_rating', 'is', null);
